...
namespace TripYari.Core.Data.DbContextScope
{
    ...
    {
        private Dictionary<Type, IUnitOfDbContext> _initializedDbContexts;
        private Dictionary<IUnitOfDbContext, IDbContextTransaction> _transactions;
        ...
        internal Dictionary<Type, IUnitOfDbContext> InitializedDbContexts { get { return _initializedDbContexts; } }
        ...
        public DbContextCollection(bool readOnly = false, IsolationLevel? isolationLevel = null, IDbContextFactory? dbContextFactory = null)
        {
            ...
            _initializedDbContexts = new Dictionary<Type, IUnitOfDbContext>();
            _transactions = new Dictionary<IUnitOfDbContext, IDbContextTransaction>();
        ...
        public TDbContext Get<TDbContext>() where TDbContext : IUnitOfDbContext
        {
            ...
            if (!_initializedDbContexts.ContainsKey(requestedType))
            {
                ...
                if (_readOnly)
                {
                    dbContext.ReadWriteDbContext.ChangeTracker.AutoDetectChangesEnabled = false;
                    if (dbContext.ReadContext == null)
                    {
                        dbContext.ReadContext.context.ChangeTracker.AutoDetectChangesEnabled = false;
                    }
                }

                if (_isolationLevel.HasValue)
                {
                    var tran = dbContext.ReadWriteDbContext.Database.BeginTransaction(_isolationLevel.Value);
