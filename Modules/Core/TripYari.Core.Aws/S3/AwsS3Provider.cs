...
namespace TripYari.Core.Aws.S3
{
    public class AwsS3Provider : IS3Provider
    {
        ...
        public AwsS3Provider(IAmazonS3 s3Client, IMetricsProvider metricsProvider, ILogger logger)
        {
            _s3Client = s3Client;
            ...
            S3StorageClass storageClass = null)
        {
            using (_metricsProvider.BeginTiming(this))
            {
                ...
                using (var writeMemoryStream = new MemoryStream())
                {
                    try
                    {
                        var objJson = JsonSerializer.Serialize(dataObject, new JsonSerializerOptions
                        {
