...
namespace TripYatri.Core.Base.Providers.Metrics
{
    public class ConsoleMetricsProvider : IMetricsProvider
    {
        ...
        public ConsoleMetricsProvider(RuntimeContext runtimeContext)
            : this(runtimeContext, TimeSpan.FromSeconds(30))
        { }

        public ConsoleMetricsProvider(RuntimeContext runtimeContext, TimeSpan flushPeriod)
        {
            Logger = new ConsoleLogger(runtimeContext);
