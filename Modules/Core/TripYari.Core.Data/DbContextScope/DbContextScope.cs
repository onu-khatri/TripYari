        ...
        private IEnumerable<EntityEntry> GetEntitiesInParentScopeToRefreshInternal(IEnumerable entities)
        {
            foreach (var contextInCurrentScope in _dbContexts.InitializedDbContexts.Values)
            {
                ...
                foreach (var toRefresh in entities)
                {
                    ...
                    var stateInCurrentScope = contextInCurrentScope.ReadWriteDbContext.Entry(toRefresh);
                    if (stateInCurrentScope != null)
                    {
                        ...
                        var source = from e in correspondingParentContext.ReadWriteDbContext.ChangeTracker.Entries()
                        ...
                        {
                            yield return correspondingParentContext.ReadWriteDbContext.Entry(entityEntry.Entity);
                        }
                    }
                }
            }
